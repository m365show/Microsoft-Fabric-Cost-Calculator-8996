...
const { 
  FiServer, FiGlobe, FiSettings, FiDollarSign, FiChevronRight, 
  FiChevronLeft, FiCheck, FiDownload, FiShare2, FiCopy 
} = FiIcons;

const MultiStepCalculator = ({ darkMode, embedded = false }) => {
      ...
      content: (
        <div className="space-y-6">
          ...
          <div className={`p-4 rounded-lg border ${
            ...
            <h5 className="font-medium mb-2">Embed URL:</h5>
            <div className="text-sm font-mono break-all text-blue-600">
              {getEmbedUrl()}
            </div>
          </div>
        </div>
  ...
  const getEmbedUrl = () => {
    ...
    const params = new URLSearchParams({
      ...
      workloads: JSON.stringify(config.workloads)
    });
    return `${baseUrl}/#/embed/multi-step?${params.toString()}`;
  };
  ...
  return (
    <div className={`${containerClass} ${
      ...
      {/* Content */}
      <div className="flex-1 p-6" style={{ minHeight: embedded ? '400px' : '500px' }}>
